...
def save_checkpoint(model, optimizer, global_step: int,
                    lr: float, input_spec: tuple, num_actions: int,
                    path: str, v_min: float = -20.0, v_max: float = 20.0, n_atoms: int = 51) -> None:
    ...
    print(f"[ckpt] Saved -> {path}  (step={global_step})")
